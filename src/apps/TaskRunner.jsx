...
import { TASK_DEFINITIONS, PRIORITIES, PRIORITY_COLORS } from '../data.js';
...
const TaskCreator = ({ taskDef }) => {
    ...
    const canCreate = hasSkilledEmployee && hasStaged && (!taskDef.needsTarget || selectedTarget.location);

    const handleSchedule = () => {
        useGameStore.getState().addEventLog(`Scheduled task: ${taskDef.description}`, "Player", "PLAYER");
...
const TaskRunner = () => {
    const { abortTask } = useGameStore.getState();
    ...
    const memoizedTaskCreators = useMemo(() => {
        return TASK_DEFINITIONS.map(taskDef => <TaskCreator key={taskDef.id} taskDef={taskDef} />);
    }, []);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-grow min-h-0">
                <div className="bg-gray-900 p-2 rounded-md overflow-y-auto">
                    ...
                    <div className="mt-2 space-y-2">
                        {memoizedTaskCreators}
                    </div>
                </div>
                <div className="bg-gray-900 p-2 rounded-md overflow-y-auto">
                    <h3 className="font-bold text-lg mb-2 sticky top-0 bg-gray-900 py-1">Task Queue</h3>
                    <div className="mt-2 space-y-2">
                        {tasks.length === 0 ? <p className="text-gray-400">No tasks in queue.</p> : [...tasks].sort((a,b) => PRIORITIES[b.priority] - PRIORITIES[a.priority]).map(task => { 
                            const emp = task.assignedTo ? employees.find(e => e.id === task.assignedTo) : null; 
                            return (
                                <div key={task.id} className="p-2 bg-gray-700 rounded-md text-sm">
                                    <div className="flex justify-between items-start">
                                        <div><strong>Task:</strong> {task.description}</div>
                                        <span className={`px-2 py-0.5 text-xs rounded-full ${PRIORITY_COLORS[task.priority]}`}>{task.priority}</span>
                                    </div>
                                    <div>Status: {task.status}</div>
                                    {emp && <div>Assigned: {emp.name}</div>}
                                    {task.status === 'In Progress' && task.completionTime && <div>ETA: {formatETA(task.completionTime)}</div>}
                                    <button onClick={() => abortTask(task.id)} className="mt-2 text-xs bg-red-700 px-2 py-1 rounded hover:bg-red-600">Abort</button>
                                </div>
                            );
                        })}
                    </div>
